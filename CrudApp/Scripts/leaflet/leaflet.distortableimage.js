...
L.EditHandle = L.Marker.extend({
    // Base class for each edit handle 
    ...
    _bindListeners: function () {
        // Called when marker is added to map
        this.on({
...
L.LockHandle = L.EditHandle.extend({
    // Handle type when edit mode is "locked"
...
L.DistortHandle = L.EditHandle.extend({
    // Handle type when edit mode is "distort"
...
L.RotateHandle = L.EditHandle.extend({
    // Handle type when edit mode is "rotate"
    options: {
        TYPE: 'rotate',
        icon: new L.Icon({
            ...
            //iconUrl: 'https://public.wmo.int/sites/all/modules/contrib/social_media_links/libraries/elegantthemes/PNG/instagram.png',
            iconSize: [16, 16],
            iconAnchor: [8, 8]
        }
		)
...
if (true) { } // TODO FIXME DELETE


L.DistortableImageOverlay = L.ImageOverlay.extend({
    // Base class for image to be distorted.
    ...
    initialize: function (url, options) {
        ...
        this._bounds = new L.latLngBounds(options.corners);
    ...
    onAdd: function (map) {
        ...
        if (this.options.editable) {
            console.log("image is editable");
            // console.log(this._image instanceof HTMLImageElement); TODO FIXME DELETE
    ...
    _initImage: function () {
        ...
        console.log("is image: ", this._image instanceof HTMLImageElement); // TODO FIXME DELETE
    ...
    _initImageDimensions: function () {
        ...
        if (this.options.corners) {
            this._corners = this.options.corners;
            this._bounds = L.latLngBounds(this._corners);
    ...
    _fireMouseEvent: function (event) {
        ...
        //TODO FIXME DELETE
        console.log("Mouse event fired: ", event.type);
    ...
    _reset: function () {
        ...
        // TODO FIXME ORIGINAL LINE --> replacement
        //translation = L.DomUtil.getTranslateString(topLeft);
        L.DomUtil.setTransform(image, topLeft);
...
L.DistortableImage.Edit = L.Handler.extend({
    ...
    addHooks: function () {
        ...
        // Initiate editor to Rotate if not in Lock.
        if (this._mode === 'lock') {
    ...
    _toggleDistort: function () {
        ...
        /* Switch mode. */
        if (this._mode != 'distort') { this._mode = 'distort'; }
